...
from command_interface import check_room_subject, execute_room_command, \
check_item_subject, execute_item_command,execute_special_cases,\
check_enemy_subject,execute_enemy_command,execute_modal_verb,check_modal_command
    ...
    if(len(words) != 0):
        ...
        if(len(words) > 1):
            ...
            if(check_room_subject(subject)):
                execute_room_command(command, subject, player1)
            ...
            elif(check_modal_command(command)):
                sayed = " ".join(words[1:])
                execute_modal_verb(command,sayed)
